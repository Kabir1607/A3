...
BATCH_SIZE = 64 # Use 64 for speed on RTX 3050
EPOCHS = 8      # Enough to see GloVe win
EMBED_DIM = 100 # Matches 100d GloVe
HIDDEN_DIM = 256
...
def train():
    ...
    strategies = {
        "GloVe": (np.load(os.path.join(PROCESSED_DIR, "emb_glove.npy")), True), # Freeze GloVe
        "TF-IDF": (np.load(os.path.join(PROCESSED_DIR, "emb_tfidf.npy")), True), # Freeze TF-IDF
        "Learned": (np.load(os.path.join(PROCESSED_DIR, "emb_learned.npy")), False) # Train from scratch
    }
    ...
    print("\n--- Starting Comparison ---")
    for name, (mat, freeze) in strategies.items():
        print(f"\n> Training Strategy: {name}")
        model = CaptionModel(len(vocab), mat, freeze_emb=freeze).to(DEVICE)
        ...
        for e in range(EPOCHS):
            ...
            if avg_val < best_overall_loss:
                best_overall_loss = avg_val
                best_name = name
                torch.save(model.state_dict(), os.path.join(RESULTS_DIR, "best_cnn_lstm.pth"))
                
        results_log.append(f"{name}: {avg_val:.4f}")

    # Log
    with open(os.path.join(RESULTS_DIR, "tuning_results.txt"), "w") as f:
        f.write("\n".join(results_log))
        f.write(f"\nWINNER: {best_name}")

    print(f"\n✅ WINNER: {best_name} (Loss: {best_overall_loss:.4f})")
